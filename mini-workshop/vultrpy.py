...
def create(vm_):
    ...
    def wait_for_default_password():
        ...
        '''
        data = show_instance(vm_['name'], call='action')
        log.error("Waiting for default password")
        log.error(pprint.pprint(data))
        if six.text_type(data.get('default_password', '')) == '':
            time.sleep(1)
            return False
        # For some reason, Vultr sets this value for a bit before the password is published
        if six.text_type(data.get('default_password', '')) == 'not supported':
            time.sleep(1)
            return False
